...
void loop() {
  ...
  if (playToggle == 0) { // and if we aren't already paused
  ...
  else if (playToggle == 1 && ((startTime - millis()) > interval_window) ) {
    bool endFile = 0;
    bool seekFile = 0;
    //bool peekFile = 0;
    ...
    newFile(file, newVM);
    /*
      if (alreadyShown(newVM) == 0) { // if we haven't shown it before. dbl check this function
        newFile(file, newVM);
      }
      // ** come back to this with mp3 player working regading triggering a new file
      else if (alreadyShown(newVM) == 1) {
        newVM = int(random(1, numFiles + 1));
        if (alreadyShown(newVM) == 0) { // if we haven't shown it before
          newFile(file, newVM);
        }
      }
      */
  ...
  /*
  // check to see if we are playing a song. If not, start the timer.
  if  (digitalRead(7) == HIGH){ //(!MP3player.isPlay()) {
    if (playToggle == 0) { // and if we aren't already paused
      playToggle = 1;
      // start timer
      startTime = millis();
    }
    else if (playToggle == 1 && ((startTime - millis()) > interval_window) ){ // if we are not playing & haven't played in a while

      newVM = int(random(0, numFiles)); // select new file
      if(!alreadyShown(newVM)){  // if we haven't shown it before
        newFile(newVM);
      }
    }
  }
  else{ //if(MP3player.isPlay()){
    startTime = 0;
    playToggle = 0;
    interval_window = int(random((timeInterval - 20,000), timeInterval));  // select a new random interval for silence
  }
  */
...
void newFile(String file, int rand_indice) {  //int rand_indice
  ...
  int screens = int(ceil(siz / 80 ));  // how many screens will we need
...
void oneScreen(String filename, int pS) {  // int lag
  for ( int i = 0; i < 4; i ++) { // each loop is one scren
    // **** is this where we start to loop for each screen ?
